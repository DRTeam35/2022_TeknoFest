...
def gudumlenme(center_2, pwm_g, frame_center, out_g, cap_g, cap_gon, Area_g,angle, move=1, last=0, pid_i=0):
    # Merkezi Tolerans
    center_tol = 100
    pressure = 20
    ...
    if center_2[0] > range3[1]:
        # sola git
        motor_function.r_donus(pwm_g, 322)
        move = 1
        last = 0.0#error
        pid_i = 0.0
    ...
    # Eğer nesneni merkezi kümenin sağındaysa sağ yengeç hareketi gerçekleştirilir
        ...
        # sağa git
        motor_function.l_donus(pwm_g, 322)
        move = 1
        last = 0.0#error
        pid_i = 0.0
        angle = control.gyro()[3]
    ...
    # -----------------------------------------------------------------------------------------------------------------#
        ...
        last, pid_i = motor_function.motor_fri(pwm_g, 351, last, pid_i, angle)
        move = 0
    return move, pressure, last, pid_i,angle
