...
class graph():
    ...
    def plot_graph(self):
        ...
        plt.xlabel('Time')
        plt.ylabel('Error')
        plt.title('Deneme_1(KP=1,KI=0.0005,KD=400)')
        plt.savefig('Error_Graph_1.png')
...
def calculate_graph(x, y, error, time):
    x.append(error)
    y.append(time)
...
def sleep_function(sec_ang, choose, cap_s, out_s, pwm_s, cap_salt, check=1, speed=350):
    ...
    if choose == 't':
        ...
        pid_i = 0.0
        graphics=graph()
        graphics.pull_data(last,Second_x)
        while Second_x <= float(sec_ang):
            ...
            """while center_g[0] != None:
                ...
                move, pressure, last, pid_i, angle = Mission.gudumlenme(center_g, pwm_s, frame_center,
                                                                        out_s, cap_salt, cap_s, Area_s,
                                                                        angle, move, last, pid_i)
...
"""
    ...
    # kaybedilirse motorlar dondurulur fakat motorlar çalışmadığından açı değişmez ve bu döngüde takılı kalır.
        ...
        angle = float(first_value[3]) + sec_ang  # Dönmesi istenilen açı değeri
            ...
            if center_g[0] != None:
                while Second_X < float(2):
                    ...
                    if center_g[0] != None:
                        ...
                        Second_X = 0
                        # merkez konumu değerlendirme amacıyla güdümlenme fonksiyonuna gönderilir
                        move, pressure, last_e, pid_i_e, angle_inst = Mission.gudumlenme(center_g, pwm_s, frame_center,
                                                                                         out_s, cap_salt, cap_s, Area_s,
