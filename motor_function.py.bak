...
def calibration(pwm):
    ...
    pwm.set_pwm(15, 0, 600)  # sağ ön dikey
    pwm.set_pwm(14, 0, 600)  # sol ön dikey
    pwm.set_pwm(13, 0, 600)  # sağ arka dikey
    pwm.set_pwm(12, 0, 600)  # sol arka dikey
    pwm.set_pwm(11, 0, 600)  # sağ ön itici
    pwm.set_pwm(10, 0, 600)  # sol ön itici
    pwm.set_pwm(9, 0, 600)  # sağ arka itici
    pwm.set_pwm(8, 0, 600)  # sol arka itici
    #Motorların sesinin bitmesine kadar geçen süre
    time.sleep(5)
    #Daha sonra Minimum PWM değeri giriliyor
    pwm.set_pwm(15, 0, 1)  # sağ ön dikey
    pwm.set_pwm(14, 0, 1)  # sol ön dikey
    pwm.set_pwm(13, 0, 1)  # sağ arka dikey
    pwm.set_pwm(12, 0, 1)  # sol arka dikey
    pwm.set_pwm(11, 0, 1)  # sağ ön itici
    pwm.set_pwm(10, 0, 1)  # sol ön itici
    pwm.set_pwm(9, 0, 1)  # sağ arka itici
    pwm.set_pwm(8, 0, 1)  # sol arka itici
    # Motorların sesinin bitmesine kadar geçen süre
    time.sleep(5)
    
    # pwm.set_pwm(channel_number, pwm_high_start, pwm_high_stop)
    #En son olarak motorların durgun olduğu orta nokta PWM'i veriliyor
    pwm.set_pwm(15, 0, 318)  # sağ ön dikey
    pwm.set_pwm(14, 0, 318)  # sol ön dikey
...
def motor_fri(pwm,p,last_error,pid_i,desired_angle):  # ileri motorlar
    ...
    left_motor,right_motor,last_error,pid_i = pid_object.calculateMotorPowers(1.00,0.0005,400.00,p,desired_angle
                                                                              ,current_yaw,delta_time)

    pwm.set_pwm(8, 0, left_motor) # sol mu sağ mı
    pwm.set_pwm(9, 0, right_motor)
    pwm.set_pwm(10, 0, left_motor)
    pwm.set_pwm(11, 0, right_motor)
...
def motor_r_yengec(pwm):  # sağ/sol motorlar
    ...
    pwm.set_pwm(11, 0, 351)  # gerekirse 0'ı 5 yap
...
def motor_l_yengec(pwm):  # sağ/sol motorlar
    ...
    pwm.set_pwm(10, 0, 351)  # gerekirse 0'ı 5 yap
    pwm.set_pwm(9, 0, 318)
    pwm.set_pwm(8, 0, 285)

def l_donus(pwm,p=325):
    fark = p - 318
    pwm2 = 318 - fark
