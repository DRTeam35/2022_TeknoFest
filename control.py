...
def Pressure_Control(pressure,pwm):
    if pressure!=0:
        ...
        if Sensor_Value > float(pressure_Down):
            motor_function.motor_u(pwm,340)
        elif Sensor_Value < float(pressure_Up):
            motor_function.motor_d(pwm,290)
        else:
...
def turn(angle,pwm):
    ...
    if angle<line+tolerans and angle>line-tolerans:
...
def zero_point(pwm,check=1):
    ...
    while check==1:
        ...
        if yaw<0:
            motor_function.l_donus(pwm)
        #Yaw değeri pozitifse sağ dönüş yapılır
        elif yaw>0:
            motor_function.r_donus(pwm)
        #Açı değeri kontrol edilir eğer açı istenile değerler arasındaysa
        #check değeri 0'a çekilir döngüden çıkılır
        check=turn(0,pwm)


"""def PID(expected_angle,gyro_yaw):
